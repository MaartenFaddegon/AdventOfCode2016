...
toInt n []     = n
toInt n (d:ds) = toInt (10*n + digitToInt d) ds

count []     = []
count (x:xs) = count' x 1 xs
count' y n (x:xs) | x == y    = count' y (n+1) xs
                  | otherwise = (n,y) : count (x:xs)
count' y n []                 = [(n,y)]

fiveMostCommon :: String -> String
fiveMostCommon = take 5 . map snd . sortBy myOrd . count . sort . filter isAlpha
...
realRoom str = fiveMostCommon (name str) == checksum str

realSectorSum = sum . map sector . filter realRoom

ex1 = "aaaaa-bbb-z-y-x-123[abxyz]"
ex2 = "a-b-c-d-e-f-g-h-987[abcde]"
ex3 = "not-a-real-room-404[oarel]"
ex4 = "totally-real-room-200[decoy]"
