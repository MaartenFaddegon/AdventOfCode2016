...
data BetweenBrackets = In | Out

subsequencesN :: Int -> [a] -> [[a]]
subsequencesN n xs | length xs >= n = take n xs : subsequencesN n (tail xs)
                   | otherwise      = []

abba = any abba' . subsequencesN 4

abba' [a,b,c,d] = a /= b && a == d && b == c

tls s = tls' Out s False

tls' _   [] b = b

tls' Out s  b = case s2 of []      -> b'
                           (_:s2') -> tls' In s2' b'
  where (s1,s2) = span (/='[') s
        b'      = abba s1 || b

tls' In s  b | abba s1   = False
             | otherwise = case s2 of []       -> b
                                      (_:s2')  -> tls' Out s2' b
  where (s1,s2) = span (/=']') s

ex1 = tls "abba[mnop]qrst"             == True
ex2 = tls "abcd[bddb]xyyx"             == False
ex3 = tls "aaaa[qwer]tyui"             == False
ex4 = tls "ioxxoj[asdfgh]zxcvbn"       == True
ex5 = tls "abba[mnop]qrsta[abba]abcd"  == False
ex6 = tls "adba[mnop]qrsta[abca]aabba" == True
